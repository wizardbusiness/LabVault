...
#### **A. Internal Functions** - Functions defined in the Templater library\

#### **B. User Functions** 
- #### i. System Commands
- #### ii. User Scripts
### The Templater `tp` object
- #### The `tp` object contains the entire library of functions and variables included with the Templater plugin. 
- #### `tp` object properties are accessed via javascript dot-notation  (ex. `tp.title`)

### Templater functions
- #### A number of internal functions are included with the Templater plugin.
- #### Internal function can be accessed and invoked on the `tp` object via dot notation in the 
	- **Access function definition**: `tp.<function>` 
	- **Invoke a function**:
		- **Function without arguments** - `tp.<function>()` 
		- **Function with arguments**: `tp.<function>(arg1, arg2, arg3)`
- #### Nested functions on the `tp` object can be accessed and invoked via dot-chaining
	- **Access nested function definition** - `tp.<child-function>.<child-function>`
	- **Invoke nested function**:
		- **Nested function without arguments** - `tp.<function>.<child-function>()`
		- **Nested function with arguments** - `tp.<function>.<child-function>(arg1, arg2, arg3`
> [!example]- **`date.now` dot-chaining example** 
>```javascript
   <% tp.date.now("YYYY-MM-DD", 7) %>
> ```
> 
> >[!example] **Decomposition**
> > - `<% %>` - Execution tag - Runs the code inside with no output. *Note: This is an example of a `delimmiter` which Templater uses to mark the boundary between markdown text and executable code inside your note.*
> > - `tp`- The Templater root object - contains the library of functions and properties included with templater. 
